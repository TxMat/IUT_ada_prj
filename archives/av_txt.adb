...
begin
        -- INIT grille
        put_line("Début de la phase d'initialisation");
        InitPartie(Grille, Pieces);
        put_line("Initialisation terminée");
        -- Demande num config
        loop
            begin
                put_line("Entrez un numéro de niveau [1-20] : ");
                get(num_conf);
                skip_line;
                exit;
            exception
        ...
        while Play loop -- pour verifier que l'user veut bien jouer
            ...
            Premier_tour := True;
            -- Debut config
            put_line("Début de la phase de configuration");
            ...
            begin -- pour gerer les exception
                    ...
                    if rep = "p" or Mode_Opti then -- Deplacement pieces
                        loop -- loop si piece invalide
                            put_line("Choisissez le numero d'une pièce à déplacer");
                                        ...
                                        if Mode_Opti then
                                            loop
                                                ...
                                                if rep = "y" then
                                                    ...
                                                    put_line("Dans ce cas mettez le numéro d'une pièce entre 0 et 8 sur la grille !");
                                                    exit;
                                                elsif rep = "a" then
                                                    ...
                                                    put_line("Répondez avec 'y' ou 'n'");
                                                end if;
                                            end loop;
                                        else
                                            put_line("Le chiffre de la couleur doit être contenu entre 0 et 8 !");
                                        end if;
                                end;
                            end loop;
                            if Pieces(coul) and coul /= T_coulP'last and checkpossible(Grille, coul) then -- si la couleur est dans la gille et n'est pas blanche on entre
                                put_line("Dans quelle direction voulez vous déplacer la pièce ? [hg / bg / bd / hd]");
                                lire(dir);
                                a_la_ligne;
                                ...
                                put_line("Les pièces blanches ne peuvent pas être déplacées");
                            elsif not checkpossible(Grille, coul) then
                                nb_err := nb_err + 1;
                                put_line("Cette pièce ne peut bouger nulle part");
                            else -- message d'erreur piece inexistante
                                put_line("Cette pièce n'existe pas dans la configuration actuelle");
                            end if;
                        end loop;
                        ...
                        put_line("-- Après" & integer'image(nb_coups) & " coups");
            ...
            if num_conf = 20 then
                if Guerison(grille) then
                    ...
                    put_line("Nous espèrons qu'il vous a plu :)");
                ...
                loop
                    ...
                    if rep = "y" then
                        ...
                        put_line("Pensez à repartir du niveau" & Integer'image(num_conf) & " pour continuer le jeu");
